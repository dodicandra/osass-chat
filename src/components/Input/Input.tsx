...
const Input: React.FC<InputProps> = ({
  ...
  return (
    <View style={containerStyle}>
      ...
      <View style={[styles.container, {borderColor: borderColor, borderWidth: borderWidth}]}>
        {phoneCode && <Text style={styles.prefix}>+62</Text>}
        {onlyText ? (
          <Text onPress={onPresText} numberOfLines={numberOfLines} style={styles.text}>
            {name}
          </Text>
        ) : (
          <>
            ...
            />
            {secureTextEntry && (
              <TouchableOpacity onPress={() => setActive(!active)} style={{marginRight: 15}}>
                <Icons.Entypo name={active ? 'eye' : 'eye-with-line'} size={30} />
              </TouchableOpacity>
            )}
          </>
        )}
